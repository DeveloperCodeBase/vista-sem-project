export default function StatsHighlights() {
  const stats = [
    { value: "18+", label: "پروژه هوش مصنوعی و GeoAI" },
    { value: "32 میلیارد ریال", label: "صرفه‌جویی سالانه برای کارفرماها" },
    { value: "97٪", label: "رضایت کارفرمایان از استقرار و پشتیبانی" },
    { value: "ISO 27001", label: "پروتکل امنیت و حاکمیت داده" }
  ];
  ...
  return (
    <section className="container relative -mt-16 mb-16">
      <div className="grid gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_30px_80px_rgba(2,6,23,0.45)] backdrop-blur-xl sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((item) => (
          <div key={item.label} className="flex flex-col items-end gap-2 rounded-2xl bg-white/5 px-4 py-5 text-right text-white/85 shadow-inner shadow-white/10">
